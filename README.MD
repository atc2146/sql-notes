...
Notes and references on SQL. Taken from various places on the internet. For educational use only.
...
* **Not** case sensitive.
* Mostly optional: Use a semi-colon at the end of each statement.
...
The statement below will only return distinct values from the "col1" columns.  Alternatively, one could use a `GROUP BY` clause (recommend to use `DISTINCT` however).
...
The `ILIKE` operator is case insenstive. Another alternative is using `lower(column_name)`
...
### LISTAGG
`LISTAGG()` / `GROUP_CONCAT()` function is an aggregate function that concatenates strings from a group into a single string with various options.

Note: `LISTAGG()` is ANSI SQL.

```sql
LISTAGG([DISTINCT] measure_column [, 'delimiter']) 
    WITHIN GROUP (ORDER BY sort_column [ASC | DESC])
...
SELECT 
    Order_ID,
    LISTAGG(Item, '; ') WITHIN GROUP (ORDER BY Item ASC) AS Shopping_List
FROM Store_Orders
GROUP BY Order_ID;
```

| Order_ID | Shopping_List |
| :--- | :--- |
| 101 | Apples; Bread; Milk |
| 102 | Coffee; Sugar |
...
WITH send_timespent AS (
    SELECT 
        ...
        SUM(activities.time_spent) AS send_timespent 
    FROM age_breakdown
...
open_timespent AS (
    SELECT 
        age_breakdown.age_bucket, 
        SUM(activities.time_spent) AS open_timespent 
    FROM age_breakdown
    LEFT JOIN activities 
        ON age_breakdown.user_id = activities.user_id
...
* Window functions are evaluated after the `WHERE`, `GROUP BY`, and `HAVING` clauses, but before the final `ORDER BY` of the query.
* Window functions operate on a set of rows and return a single value for *each* row from the underlying query.
* `PARTITION BY` column, and `ORDER BY` column are optional; i.e if no partiton, taken over whole table
  * `ORDER BY` is not optional for `ROW_NUMBER()`, `RANK()`, `DENSE_RANK()`, `NTILE()`
...
```sql
-- 3-Day Moving Average (includes current row and 2 previous days)
SELECT
  Ticker,
  Date,
  Price,
...
`ROWS` is a sliding window by row count (doesn’t care about duplicates in `ORDER BY`).
...
Example:
```sql
-- Average across a sliding 3-row window (previous, current, next)
SELECT
  AVG(price) OVER (
...
...
```
For ranking functions (`ROW_NUMBER()`, `RANK()`, etc.), the frame clause is ignored—they always operate on the entire partition.
...
SELECT 'c' AS v FROM dual UNION ALL
SELECT 'c' AS v FROM dual UNION ALL
SELECT 'd' AS v FROM dual UNION ALL
SELECT 'e' AS v FROM dual;
...
SELECT
  ...
  ROW_NUMBER() OVER (PARTITION BY v ORDER BY v) row_number_dedupe
FROM t
ORDER BY v;

+---+------------+------+------------+------------------+
| V | ROW_NUMBER | RANK | DENSE_RANK | ROW_NUMBER_DEDUPE|
+---+------------+------+------------+------------------+
| a |          1 |    1 |          1 |                1 |
| a |          2 |    1 |          1 |                2 |
| a |          3 |    1 |          1 |                3 |
...
* Gotchas: For boolean expressions, be very careful when using a combination of `OR` and `AND` statements. For example, `(expr1 OR expr2) AND expr3` is different than `expr1 OR expr2 AND expr3` .  
